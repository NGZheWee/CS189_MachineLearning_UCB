...
class ReLU(Activation):
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        """
        ### YOUR CODE HERE ###
        # Derivative of ReLU is 1 for Z > 0, and 0 otherwise
...
class SoftMax(Activation):
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        # Compute the Jacobian matrix for each example in the batch
        for i in range(Z.shape[0]):
            # Get the softmax vector for the i-th sample
            s = softmaxOutput[i]

            # Compute the Jacobian matrix of the softmax function
            jacobian = np.diagflat(s) - np.outer(s, s)

            # Multiply by the upstream gradient (dY) to get dZ
